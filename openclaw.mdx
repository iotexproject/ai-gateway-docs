...
## Quick Setup (Ask Your Agent)

If you already have OpenClaw running, just tell your agent:

```
Add gateway.iotex.ai as an OpenAI-compatible LLM provider with API key sk-xxxxxxxxxx.
Use "openai-completions" as the API type and "iotex" as the provider name.
Set the base URL to https://gateway.iotex.ai/v1.
Add gemini-2.5-flash-lite as an available model.
Docs: https://docs.iotex.ai/
```

Get your API key from the [Gateway Console](https://gateway.iotex.ai/console/token) first.

<Warning>
For audio transcription, **do not** rely on the agent to configure it automatically. The media understanding system requires a specific provider mapping. Follow the [Audio Transcription](#audio-transcription) section below for manual setup.
</Warning>
...
See the full list of models available through IoTeX AI Gateway on the [Supported Models](/supported-models) page. Popular choices for OpenClaw:

| Model | Best For |
|-------|----------|
| `gemini-2.5-flash-lite` | Low-cost general chat, fast responses |
| `gemini-2.5-flash` | Balanced quality and speed |
| `deepseek-ai/DeepSeek-V3-0324` | Complex reasoning |
| `Qwen/Qwen3-30B-A3B` | Chinese language, general tasks |
